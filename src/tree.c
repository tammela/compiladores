...
Exp *newexpNode(Exp *ep, Type *t)
{
   ...
   e->newexp.expt = seqtypeNode(SEQ, t);
   e->newexp.size = ep;
...
Exp *intexpNode(int i)
{
   ...
   e->intexp.val = i;
   e->intexp.expt = typeNode(ATOMIC, INT);
...
Exp *fltexpNode(float f)
{
   ...
   e->fltexp.flt = f;
   e->fltexp.expt = typeNode(ATOMIC, FLOAT);

   return e;
}

Exp *strexpNode(char *str, int len)
{
   Exp *e = myalloc(sizeof(Exp));
   ...
   e->strexp.str = str;
   e->strexp.len = len;
   e->strexp.expt = seqtypeNode(SEQ, typeNode(ATOMIC, CHAR));
